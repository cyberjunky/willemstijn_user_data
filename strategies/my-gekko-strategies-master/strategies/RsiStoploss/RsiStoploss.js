...
method.check = function (candle) {
  ...
  //console.debug('Novo candle', rsi.lastClose.toFixed(digits), rsiVal)
  ...
  if (rsiVal > this.settings.thresholds.high) { // Venda
    ...
    if (!this.stop.buy) {
      ...
      //   this.stop.buy = false;
      //   this.advice('short');
      //   console.debug('short a => ', candle.vwp.toFixed(digits))
      // } else
      this.advice();
    } else {
      this.advice();
    ...
    if (!this.stop.buy) {
      ...
      if (this.trend.persisted && !this.trend.adviced) {
        this.trend.adviced = true;
        this.advice('long');
        console.debug('Compra - long a => ', candle.close.toFixed(digits))
        this.stop.buy = true;
        ...
        console.debug('Limit STOP: ', this.stop.stopValue.toFixed(digits), 'GAIN: ', this.stop.gainValue.toFixed(digits))
      } else
        this.advice();
    }
    else {
    ...
    if (this.stop.buy) { //Se estou comprado
      ...
      if (this.candle.close <= this.stop.stopValue) { //Venda atingiu o stop
        ...
        this.stop.buy = false;
        this.advice('short');
        console.debug('Venda através do stop(LOW) => ', candle.vwp.toFixed(digits))
        ...
        if (this.stop.progressiveGain) { //Se o stop é progressivo
          ...
          this.stop.gainValue = this.stop.buyValue + (this.stop.buyValue * (this.stop.gain / 100));
          this.advice();
          console.debug('Recalculado o novo limit STOP: ', this.stop.stopValue.toFixed(digits), 'GAIN: ', this.stop.gainValue.toFixed(digits))
        } else { // Se não for progressivo - Venda com o ganho atingido
          this.trend.adviced = true;
          this.stop.buy = false;
          this.advice('short');
          console.debug('Venda através do stop(GAIN) => ', candle.vwp.toFixed(digits))
        }
