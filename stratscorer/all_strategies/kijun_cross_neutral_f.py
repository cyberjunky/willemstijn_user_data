# ==============================================================================================
# Neutral Kijun cross strategy for Futures
#
# Made by:
...
# freqtrade backtesting -c user_data/futures_config.json -s kijun_cross_neutral_f --timerange=20190101-20210530 --timeframe=1d
# freqtrade backtesting-analysis
# freqtrade plot-dataframe -p BTC/USDT --strategy kijun_cross_neutral_f -c user_data/spot_config.json
#
...
class kijun_cross_neutral_f(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # First: kijun should be above senkou_a OR above senkou_b (so in between) to make the signal TRUE
        dataframe["kijun_within_cloud_l"] = (dataframe["kijun"] > dataframe["senkou_a"]) | (dataframe["kijun"] > dataframe["senkou_b"])
        # Second: close should be above senkou_a OR above senkou_b (so in between) to make the signal TRUE
        dataframe["close_within_cloud_l"] = (dataframe["close"] > dataframe["senkou_a"]) | (dataframe["close"] > dataframe["senkou_b"])
        ...
        # The buy long signal can only be True if the kijun_within_kumo, close_within_kumo and close_above_kijun are all True
        dataframe["buy_long"] = (
            (dataframe["kijun_within_cloud_l"] == True)
            & (dataframe["close_within_cloud_l"] == True)
            & (dataframe["close_above_kijun"] == True)
        )

        # The sell short signals are similar to buy long, but reversed
        dataframe["kijun_within_cloud_s"] = (dataframe["kijun"] < dataframe["senkou_a"]) | (dataframe["kijun"] < dataframe["senkou_b"])
        dataframe["close_within_cloud_s"] = (dataframe["close"] < dataframe["senkou_a"]) | (dataframe["close"] < dataframe["senkou_b"])
        dataframe["close_below_kijun"] = dataframe["close"] < dataframe["kijun"]
        dataframe["sell_short"] = (
            (dataframe["kijun_within_cloud_s"] == True)
            & (dataframe["close_within_cloud_s"] == True)
