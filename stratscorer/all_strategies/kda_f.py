...
# freqtrade backtesting -c user_data/futures_config.json -s kda_f --timerange=20190101-20210530 --timeframe=1d
# freqtrade backtesting -c user_data/futures_config.json -s kda_f --timerange=-20230101 --timeframe=1d
# freqtrade hyperopt -c user_data/futures_config.json -s kda_f --epochs 50 --spaces sell --random-state 8105 --min-trades 10 --hyperopt-loss SharpeHyperOptLoss
# freqtrade backtesting-analysis
# freqtrade plot-dataframe -p BTC/USDT:USDT --strategy kda_f -c user_data/futures_config.json
# ==============================================================================================
...
class kda_f(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Uncomment this for dataframe analysis
        # print(self)
        # print(metadata)
        # print(dataframe)
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                # The same goes for short signals, either there is a takeprofit signal
                # or a stoploss signal.
                # In case of short trades the close price should be below the takeprofit point
                (dataframe["close"] < dataframe["takeprofit"])
                # & (dataframe["signal"] == 'short')
                & (dataframe["volume"] > 0)  # Guard
            ),
            ["exit_short", "exit_tag"],
        ] = (1, "takeprofit_short_exit")

        dataframe.loc[
            (
                # The same goes for short signals, either there is a takeprofit signal
                # or a stoploss signal.
                # In case of stoploss for shorts, the close is higher than the stoploss
                (dataframe["close"] > dataframe["stoploss"])
                & (dataframe["signal"] == 'short')
                & (dataframe["volume"] > 0)  # Guard
            ),
            ["exit_short", "exit_tag"],
        ] = (1, "stoploss_short_exit")

        # # Just an additional idea that I left behind
        # dataframe.loc[
        #     (
        #         # There is one final exit signal that indicates that the momentum is over
        #         # and the chance of getting profits is negligable. That is when the KST get's above its KST signal line
        #         (dataframe["kst"] > dataframe["ksts"])
        #         # & (dataframe["signal"] == 'long')
        #         & (dataframe["volume"] > 0)  # Guard
        #     ),
        #     ["exit_long", "exit_tag"],
        # ] = (1, "momentum_short_passed_exit")
